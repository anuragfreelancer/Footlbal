...
const getSessionDateStr = (session: any): string => {
  const dateStr = session?.session_start_date ?? '';
  if (typeof dateStr !== 'string') return '';
  if (looksLikeDate(dateStr)) return dateStr.trim();
  const d = session?.session_start_time ?? dateStr;
  if (typeof d !== 'string') return '';
  const parts = d.split(' ');
  return parts[0] || d;
};

const getSessionType = (s: any): string =>
  String(s?.type || s?.session_type || 'TRAINING').toUpperCase();
...
const useCalendar = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const sessionData = sessionRes?.userGetData ?? [];
    ...
    return list
      .filter((player: any) => {
        return coachSessions.some((s: any) => {
          const dateStr = getSessionDateStr(s);
          return looksLikeDate(dateStr) && dateStr === selectedDates && String(s.user_id) === String(player.id);
        });
      })
      .map((player: any) => {
        const matchingSession = coachSessions.find((s: any) => {
          const dateStr = getSessionDateStr(s);
          return looksLikeDate(dateStr) && dateStr === selectedDates && String(s.user_id) === String(player.id);
        });
        const sessionType = matchingSession
          ? String(matchingSession?.type || matchingSession?.session_type || "TRAINING").toUpperCase()
          : "TRAINING";
        return { ...player, sessionType };
      });
  }, [players, sessions, selectedDates]);
