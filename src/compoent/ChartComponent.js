...
const ChartComponent = ({ data, statusText, statusColor }) => {
  ...
  const currentData = data[selectedType].data;
  const maxValue = Math.max(...currentData);
  const fullWeekLabels = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
const currentDayIndex = moment().isoWeekday(); // Monday = 1, Tuesday = 2 ... Sunday = 7
const weekData = [5, 10, 30, 45, 5, 20, 15]; // Sample data for full week

// Monday se current day tak ka data filter karna
const filteredLabels = fullWeekLabels.slice(1, currentDayIndex + 1); // Monday se aaj tak ke din
const filteredData = weekData.slice(1, currentDayIndex + 1); // Sirf usi din ka data jo show karna hai
  ...
  return (
    <View style={{ marginVertical:1,marginHorizontal:1,marginBottom:20,marginTop:12,  backgroundColor: "#fff", borderRadius: 15, padding: 10, shadowColor: "#000", shadowOpacity: 0.1, shadowRadius: 5, elevation: 1 }}>
      ...
      <LineChart
  data={{
    labels: filteredLabels, // Sirf Monday se aaj tak ke din dikhayenge
    datasets: [
      {
        data: filteredData, // Sirf Monday se aaj tak ka data
        color: (opacity = 1) => `rgba(255, 99, 132, ${opacity})`,
        strokeWidth: 2,
      },
    ],
  ...
  chartConfig={{
    ...
    color: (opacity = 1) => `rgba(255, 99, 132, ${opacity})`,
    labelColor: (opacity = 1, index) =>
      index === filteredLabels.length - 1 ? "blue" : "#7B6F72", // Current day ka label blue
    propsForLabels: {
      fontSize: 12,
      fontWeight: "600",
    },
    propsForDots: {
      r: "4", // Dots enable kiye current data ke liye
    },
  }}
  bezier
  style={{ marginVertical: 8, borderRadius: 16 }}
